        ...
        case 0x05: {
            ...
            switch (instr->dp_opc) {
                case 0x00: // MOVN
                    ...
                    // For 32-bit MOVN, result should be ~imm shifted into lower 32-bits, upper 32-bits zero.
                    // set_register_value handles the Wn clearing upper bits behavior.
                    break;
                case 0x02: // MOVZ
...
static void execute_dp_reg_instruction(ARMState* state, const DecodedInstruction* instr) {
    ...
    if (instr->dp_reg_M == 0) {
        ...
        if (instr->dp_reg_N == 1 && (instr->dp_reg_opr >> 3) == 0) { // N bit is part of opr for logical
        ...
        if (! ( (instr->dp_reg_opr >> 3) & 0x1 ) ) { // If bit 3 of opr is 0, it's logical
            ...
            if (set_flags || instr->dp_rd == 31) { // If S bit is set, or if Rd is ZR (like TST, CMP)
                // TODO update pstate
            }
            set_register_value(state, instr->dp_rd, result, sf);
